import styled from "styled-components";
import { useApp } from "../../hooks";
import logo from "../../assets/Admin/book.svg";

import { v } from "../../styles/Variables";
import {
  AiOutlineLeft,
  AiOutlineHome,
  AiOutlineApartment,
  AiOutlineSetting,
  AiOutlineUser,
  AiOutlineSchedule,
  AiOutlineCheckSquare,
  AiOutlineCheck,
  AiOutlineCheckCircle,
  AiOutlineCalendar,
  AiOutlineUserSwitch,
  AiOutlineUsergroupAdd,
} from "react-icons/ai";
import {
  MdOutlineAnalytics,
  MdLogout,
  MdOutlineBook,
  MdCategory,
  MdOutlineCategory,
  MdOutlineBookmark,
} from "react-icons/md";
import { NavLink } from "react-router-dom";


export const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  const ModSidebaropen = () => {
    setSidebarOpen(!sidebarOpen);
  };

  const { theme, setTheme, logout } = useApp();
  const CambiarTheme = () => {
    setTheme((theme) => (theme === "light" ? "dark" : "light"));
  };
  return (
    <StyledComponent isOpen={sidebarOpen} themeUse={theme}>
      <button className="Sidebarbutton" onClick={ModSidebaropen}>
        <AiOutlineLeft />
      </button>
      <div className="Logocontent">
        <div className="imgcontent">
          <img src={logo} />
        </div>
        <h2>Biblioteca</h2>
      </div>
      {linksArray.map(({ icon, label, to }) => (
        <div className="LinkContainer" key={label}>
          <NavLink
            to={to}
            className={({ isActive }) => `Links${isActive ? ` active` : ``}`}
          >
            <div className="Linkicon">{icon}</div>
            {sidebarOpen && <span>{label}</span>}
          </NavLink>
        </div>
      ))}
      <Divider />

      {secondarylinksArray.map(({ icon, label, to }) => (
        <div className="LinkContainer" key={label}>
          <NavLink
            to={to}
            className={({ isActive }) => `Links${isActive ? ` active` : ``}`}
            onClick={logout}
          >
            <div className="Linkicon">{icon}</div>
            {sidebarOpen && <span>{label}</span>}
          </NavLink>
        </div>
      ))}
      <Divider />
      <div className="Themecontent">
        {sidebarOpen && <span className="titletheme">Tema</span>}
        <div className="Togglecontent">
          <div className="grid theme-container">
            <div className="content">
              <div className="demo">
                <label className="switch" istheme={theme}>
                  <input
                    istheme={theme}
                    type="checkbox"
                    className="theme-swither"
                    onClick={CambiarTheme}
                  ></input>
                  <span istheme={theme} className="slider round"></span>
                </label>
              </div>
            </div>
          </div>
        </div>
      </div>
    </StyledComponent>
  );
};
//#region Data links
const linksArray = [
  {
    label: "Dashboard",
    icon: <AiOutlineHome />,
    to: "/admin/dashboard",
  },
  {
    label: "Reservas",
    icon: <AiOutlineCalendar />,
    to: "/admin/reservas",
  },
  {
    label: "Préstamos",
    icon: <AiOutlineCheckCircle />,
    to: "/admin/prestamos",
  },
  {
    label: "Usuarios",
    icon: <AiOutlineUser />,
    to: "/admin/usuarios",
  },
  {
    label: "Documentos",
    icon: <MdOutlineBook />,
    to: "/admin/documentos",
  },
  {
    label: "Categorias",
    icon: <MdOutlineCategory />,
    to: "/admin/categorias",
  },
  {
    label: "Autores",
    icon: <AiOutlineSchedule />,
    to: "/admin/autores",
  },

  {
    label: "Editoriales",
    icon: <MdOutlineBookmark />,
    to: "/admin/editoriales",
  },
];
const secondarylinksArray = [
  {
    label: "Cerrar Sesión",
    icon: <MdLogout />,
    to: "/login",
  }
];
const StyledComponent = styled.div`
  color: ${(props) => props.theme.text};
  background: ${(props) => props.theme.bg};
  position: sticky;
  padding-top: 20px;
  .Sidebarbutton {
    position: absolute;
    top: ${v.xxlSpacing};
    right: -18px;
    width: 32px;
    height: 32px;
    border-radius: 50%;
    background: ${(props) => props.theme.bgtgderecha};
    box-shadow: 0 0 4px ${(props) => props.theme.bg3},
      0 0 7px ${(props) => props.theme.bg};
    display: flex;
    align-items: center;
    justify-content: center;
    cursor: pointer;
    transition: all 0.3s;
    transform: ${({ isOpen }) => (isOpen ? `initial` : `rotate(180deg)`)};
    border: none;
    letter-spacing: inherit;
    color: inherit;
    font-size: inherit;
    text-align: inherit;
    padding: 0;
    font-family: inherit;
    outline: none;
  }
  .Logocontent {
    display: flex;
    justify-content: ${({ isOpen }) => (isOpen ? `start` : `center`)};
    align-items: center;
    gap: 5px;
    padding: 10px 0 25px 0;
    padding-left: ${({ isOpen }) => (isOpen ? `24px` : `0`)};
    
    .imgcontent {
      display: flex;
      img {
        max-width: 100%;
        height: auto;
      }
      cursor: pointer;
      transition: all 1s;
      transform: ${({ isOpen }) => (isOpen ? `scale(0.9)` : `scale(1.5)`)};
    }
    h2 {
      display: ${({ isOpen }) => (isOpen ? `block` : `none`)};
      color: ${({ themeUse }) =>
        themeUse === "light" ? `` : `#EAFF6A`} !important;
      font-weight: 600;
    }
  }
  .LinkContainer {
    margin: 8px 0;
    padding: 0 10%;
    display: flex;
    justify-content: ${({ isOpen }) => (isOpen ? `start` : `center`)};

    .Links {
      display: flex;
      align-items: center;
      text-decoration: none;
      padding: calc(${v.smSpacing}-2px) 0;
      color: ${(props) => props.theme.text};
      height: 40px;
      font-size: 16px;
      font-weight: 500;
      width: ${({ isOpen }) => (isOpen ? `100%` : ``)};
      :hover {
        background: ${(props) => props.theme.bg3};
        border-radius: 10px;
      }
      .Linkicon {
        padding: 8px;
        display: flex;

        svg {
          font-size: 22px;
        }
      }
      &.active {
        /* color: #f07613; */
        font-weight: 600;
        background: ${(props) => props.theme.bgorange};
        border-radius: 10px;
        color: #fff;
        transition: all 0.5s ease-in-out;
        .Linkicon {
          svg {
            color: #fff;
          }
        }
      }
    }
  }
  .Themecontent {
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0 10%;
    .titletheme {
      display: block;
      padding: 10px;
      font-weight: 700;
      opacity: ${({ isOpen }) => (isOpen ? `1` : `0`)};
      transition: all 0.3s;
      white-space: nowrap;
      overflow: hidden;
    }
    .Togglecontent {
      margin: ${({ isOpen }) => (isOpen ? `auto 40px` : `auto 40px`)};
      width: 73px;
      height: 33px;
      border-radius: 10px;
      transition: all 0.5s;
      position: relative;
      .theme-container {
        background-blend-mode: multiply, multiply;
        transition: 0.4s;
        .grid {
          display: grid;
          justify-items: center;
          align-content: center;
          height: 100vh;
          width: 100vw;
          font-family: "Lato", sans-serif;
        }
        .demo {
          font-size: 32px;
          .switch {
            position: relative;
            display: inline-block;
            width: 60px;
            height: 34px;
            .theme-swither {
              opacity: 0;
              width: 0;
              height: 0;
              &:checked + .slider:before {
                left: 4px;
                content: "🌑";
                transform: translateX(26px);
              }
            }
            .slider {
              position: absolute;
              cursor: pointer;
              top: 5px;
              left: -27px;
              right: 35px;
              bottom: 0;
              background: ${({ themeUse }) =>
                themeUse === "light" ? v.lightcheckbox : v.darkcheckbox};
              height: 23px;

              transition: 0.4s;
              &::before {
                position: absolute;
                content: "☀️";
                font-size: 26px;
                height: 0px;
                width: 0px;
                left: -10px;
                top: 12px;
                line-height: 0px;
                transition: 0.4s;
              }
              &.round {
                border-radius: 34px;

                &::before {
                  border-radius: 50%;
                }
              }
            }
          }
        }
      }
    }
  }
  @media (max-width: 768px) {
    display: none;
  }

`;
const Divider = styled.div`
  height: 1px;
  width: 100%;
  background: ${(props) => props.theme.bg3};
  margin: ${v.lgSpacing} 0;
`;
